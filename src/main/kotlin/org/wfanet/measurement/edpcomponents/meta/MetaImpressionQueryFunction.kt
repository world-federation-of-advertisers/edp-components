...
class MetaImpressionQueryFunction(
   ...
   * be expressed (→ `FILTER_NOT_SUPPORTED`). An empty expression means "no filter" (all
   * impressions).
   *
  ...
  private fun translateFilter(expression: String): MetaDemographicFilter? =
    if (expression.isEmpty()) MetaDemographicFilter.UNFILTERED else null
  ...
  companion object {
     ...
     *
     * TODO(@jojijacob): Load the token from Secret Manager (resource name via an env var) instead
     *   of the process environment. The `META_ACCESS_TOKEN` env fallback is for local/testing only;
     *   credentials must never leave this function's environment.
     */
    private fun defaultInsightsClient(): MetaInsightsClient {
      ...
      return MetaMarketingApiInsightsClient(accessToken = token)
    }
  }
}
